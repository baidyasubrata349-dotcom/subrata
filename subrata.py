...
API_BASE = "https://anishexploits.site/api/api.php?key=anish&num="
BOT_TOKEN = "8326492278:AAFbF7zLEwqKZk28e3jwCKfE9N03G_x6XYU"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = "👋 *WELCOME TO SAFE NET FIX TEAM*\n\n" \
                   "\n" \
                   ""
...
async def search_number_api(number):
    ...
    try:  
        ...
        if response.status_code != 200:  
            return "🛡️ I AM SUBRATA FOUNDER & OWNER OF THE SAFE NET FIX TEAM 🛡️\n\n" \
                   "🎯 TARGET: {}\n\n" \
                   "❌ DATABASE ERROR\n\n" \
...
def format_cybersecurity_report(user_data, number, record_count, current_time):
    ...
    # First check if circle contains state info
    actual_circle = 'Unknown'
    if circle:
        # Circle usually contains both state and operator
        # Example: "BIHAR JIO", "DELHI VODAFONE", etc.
        parts = circle.split()
        if len(parts) >= 2:
            # Take first part as state
    ...
    if 'JIO' in circle.upper():
        ...
        # Try to extract operator from circle
    ...
    # Build the report
    report = "🛡️ I AM SUBRATA FOUNDER & OWNER OF THE SAFE NET FIX TEAM 🛡️\n\n"
    ...
    report += f"├─ 🛡️ SUBRATA\n"
    report += f"├─ 👨‍💻 Developer: @subratahacker\n"
...
def main():
    print("\n" * 2)
    print("=" * 50)
    print("I AM SUBRATA FOUNDER & OWNER OF THE SAFE NET FIX TEAM")
